...
import { sendMessage, deleteMessage, createReport } from "@/app/_service/database-service";
...
export default function TextChannel({messages, users, channel, reportWindowOpen, setReportWindowOpen}){  
  ...
  function tryDeleteMessage(userid, messageid){
    if(userid == user.uid){
  ...
  function report(userid, messageid, messageText){
    console.log(`report: ${userid}, ${messageid}, ${messageText}`);
    setReportValues([userid, messageid, messageText]);
  ...
  function submitReport(){
    const today = new Date().getTime();
    createReport(reportValues[0], user.uid, reportValues[1], reportValues[2], reportText, today, channel);
    closeReport();
  }
        ...
        <div className="grow flex flex-col-reverse h-full overflow-y-auto">
          <div className="w-auto h-full m-5 border-2 border-neutral-600 overflow-y-scroll">
            ...
            <div className="grow p-5 flex flex-col h-11/12 justify-end">
              ...
              <div className="border border-neutral-700 rounded-xl h-16">
                <input
                  type="text"
                  value={reportText}
              ...
              if(msg.userID == user.uid){
                return(
                  <div key={msg.id} className="border-t border-neutral-800">
                    ...
                    text={msg.text}
                    date={date}
                    deleteMessage={()=>tryDeleteMessage(msg.userID, msg.id)}
                    key={msg.id}/>
                  </div>
                )
              }else{
                return(
                  <div key={msg.id} className="border-t border-neutral-800">
                    ...
                    text={msg.text}
                    date={date}
                    reportMessage={()=>report(msg.userID, msg.id, msg.text)}
                    key={msg.id}/>
                  </div>
                )
              }
          ...
          <div className="flex flex-row w-auto h-full">
            <div className="border border-neutral-700 rounded-xl p-2 grow">
              <input
                ...
                className="size-full placeholder:text-xl placeholder:italic font-thin"
              />
            </div>
            <div className="self-center w-1/12 min-w-10 min-h-16 h-full border-3 border-blue-800 rounded-xl ml-2 bg-blue-500 hover:bg-blue-400">
              <div className="size-full items-center justify-center text-blue-200 flex-nowrap grid grid-cols-1 grid-rows-1">
